...
function PlanCard({ item }: { item: RsuPlanListItem }) {
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <CardTitle className="text-base">
              {plan.name}{" "}
              <span className="font-normal text-muted-foreground">({plan.ticker})</span>
            </CardTitle>
        ...
        {valuation ? (
          <div className="grid grid-cols-2 gap-2 text-xs tabular-nums sm:grid-cols-3 lg:grid-cols-5">
            <div>
              ...
              <p className="font-medium">{formatMoneyBase(valuation.receivedGrossUsd, "USD")}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{formatMoneyBase(valuation.totalGrantGrossUsd, "USD")}</p>
            </div>
          </div>
        ) : (
        ...
        <Collapsible open={expanded} onOpenChange={setExpanded}>
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            {summary.nextVestDate ? (
              <span>
                Next vest: {new Date(summary.nextVestDate).toLocaleDateString()}
              </span>
            ) : null}
          ...
          <CollapsibleContent>
            <InsetPanel className="mt-2 p-2">
              {isPending ? (
                ...
                <p className="text-xs text-muted-foreground">No vest events.</p>
              )}
            </InsetPanel>
          </CollapsibleContent>
        </Collapsible>
  ...
  return (
    <div className="space-y-6">
      ...
      {isPending ? (
        ...
        <p className="text-sm text-muted-foreground">No RSU plans yet.</p>
      )}
    </div>
  );
}
