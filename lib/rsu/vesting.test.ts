...
describe("valuePlan", () => {
  ...
  it("returns null when quote is missing or non-positive", () => {
    const vests = [
      {
        sequence: 1,
        scheduledDate: new Date(),
    ...
    expect(valuePlan({ totalShares: 100, ticker: "AAPL" }, vests, null, 20)).toBeNull()
    expect(
      valuePlan(
        { totalShares: 100, ticker: "AAPL" },
        vests,
        { priceUsd: 0, asOf: new Date().toISOString() },
        20,
      ),
    ).toBeNull()
  })

  it("derives net shares from tax withhold when not stored on vest", () => {
    const vests = [
      {
        sequence: 1,
        scheduledDate: new Date(),
...
describe("buildVestSchedule errors", () => {
  it("rejects non-positive vest interval or period", () => {
    expect(() =>
      buildVestSchedule({
    ...
    ).toThrow("Vesting period and interval must be positive")
    expect(() =>
      buildVestSchedule({
        grantDate: new Date("2022-01-01"),
        totalShares: 100,
        vestingPeriodMonths: 48,
        vestIntervalMonths: 0,
        vestDayOfMonth: 20,
      }),
    ).toThrow("Vesting period and interval must be positive")
  })

  it("rejects period not divisible by interval", () => {
    expect(() =>
      buildVestSchedule({
        grantDate: new Date("2022-01-01"),
        totalShares: 100,
        vestingPeriodMonths: 50,
        vestIntervalMonths: 3,
        vestDayOfMonth: 20,
      }),
    ).toThrow("Vesting period must be divisible by vest interval")
  })
})

describe("summarizePlan edge cases", () => {
  it("returns zero percent complete when plan has no shares", () => {
    const s = summarizePlan({ totalShares: 0, ticker: "AAPL" }, [])
