...
export function grossMonthlyCrcFromInput(
  ...
  const inCrc = inputCurrency === "CRC" ? grossInput : grossInput * Math.max(crcPerUsd, 1e-9)
 ...
 */
export function plannedNetCrcToMonthlyIncomeBase(params: { netMonthlyCrc: number }): number {
