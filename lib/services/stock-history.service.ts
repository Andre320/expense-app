...
import { parseStockRange, STOCK_RANGE_CONFIG, type StockRange } from "@/lib/stock-range"
import {
  buildStockForecast,
...
/** @deprecated Use getStockHistory(ticker, range) */
export async function getStockHistoryByDays(ticker: string, days = 90) {
  const range: StockRange = days <= 3 ? "day" : days <= 14 ? "week" : days <= 60 ? "month" : "year"
