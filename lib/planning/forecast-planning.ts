...
import { plannedNetCrcToMonthlyIncomeBase } from "@/lib/income/tax-calculator"
...
export function goalsForForecast(
  ...
  return goals.map((g) => {
    ...
    return {
      ...
      targetAmount:
        g.targetAmount == null ? null : amountToReportingBase(g.targetAmount, cur, crcPerUsd),
    }
  })
}
...
export function savingsGoalMilestones(
  ...
  for (const g of sorted) {
    ...
    const gap = target == null ? null : Math.max(0, roundMoney(target - g.currentAmount))
...
export function liveNetCrcToExpectedIncomeBase(params: { netMonthlyCrc: number }): number {
